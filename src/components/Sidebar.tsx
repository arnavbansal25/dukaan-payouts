...
export function Sidebar() {
  return (
    <header className="bg-secondary sticky top-0 hidden h-screen w-[14rem] flex-col p-4 lg:flex">
        ...
        <ul className="flex flex-col gap-1">
          {sidebarConfig.map((item) => (
            <li className="text-sm font-medium text-white/80" key={item.name}>
              <a
                href="#"
                className={`flex items-center gap-3 rounded px-4 py-2 hover:bg-white/5
                  ${
